/*
 * @lc app=leetcode.cn id=2045 lang=javascript
 *
 * [2045] 到达目的地的第二短时间
 *
...
var secondMinimum = function (n, edges, time, change) {
  ...
  // 从1到i：path[i] = [最短，次短]
  const path = new Array(n + 1).fill(0).map(() => [Number.MAX_VALUE, Number.MAX_VALUE]);
  path[1][0] = 0;
  // queue: [[index, shortest], ...]
  const queue = []
  queue.push([1, 0]);
  while (path[n][1] === Number.MAX_VALUE) {
    const [index, len] = queue.shift();
    G[index].forEach(next => {
      if (path[next][0] > len + 1) {
        path[next][0] = len + 1;
        queue.push([next, len + 1]);
      } else if (path[next][0] < len + 1 && path[next][1] > len + 1) {
        path[next][1] = len + 1;
        queue.push([next, len + 1]);
      }
    })
