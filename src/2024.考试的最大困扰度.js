...
var maxConsecutiveAnswers = function(answerKey, k) {
  ...
  let numOfT = 0, numOfF = 0;
  let condition = () => numOfT > k && numOfF > k;
  while (right < answerKey.length) { 
    if (answerKey[right] === 'T') numOfT++;
    if (answerKey[right] === 'F') numOfF++;
    while (condition()) {
      if (answerKey[left] === 'T') numOfT--;
      if (answerKey[left] === 'F') numOfF--;
      left++
    }
